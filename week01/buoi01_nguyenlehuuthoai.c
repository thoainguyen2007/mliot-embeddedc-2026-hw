...
void parse_config(const uint8_t *config_packet, int16_t *high_threshold) {
    ...
    *high_threshold=(int16_t)(config_packet[0] | ((uint16_t)config_packet[1] << 8));
    //gs config_packet[1] là xxxx xxxx ==> (uint16_t)config_packet[1] là 0000 0000 xxxx xxxx 
    // ==>((uint16_t)config_packet[1] << 8)) là xxxx xxxx 0000 0000
    //   tương tự (int16_t)(config_packet[0] là 0000 0000 yyyy yyyy
    // Do x|0=x nên ta có kết quả là xxxx xxxx yyyy yyyy (có thể dùng + thay vì |)
...
int16_t read_temperature_reg(void *hw_sensor_reg) {
    ...
    // (int16_t*) hw_sensor_reg: ép kiểu con trỏ
    // *(int16_t*) hw_sensor_reg: giá trị con trỏ
    // *(volatile int16_t*) hw_sensor_reg: thêm volatile để tránh compiler tối ưu hóa khi dùng vòng while
...
void control_output(uint8_t *control_reg, uint8_t fan_enable, uint8_t alarm_enable) {
    ...
    if (fan_enable==1){*control_reg |=(1 << 0);} 
    else {*control_reg &= ~(1 << 0);}

    if (alarm_enable==1){*control_reg |=(1 << 1);} 
